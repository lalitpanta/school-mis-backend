...
class AccountsService {
  ...
  async getOverview(filters, req) {
    ...
    const [
      ...
      // Students billed
      db
        .query(`SELECT COUNT(DISTINCT student_id) AS cnt FROM student_fees`)
        .catch(() => ({ rows: [{ cnt: 0 }] })),
      ...
      db
        .query(
          `SELECT COUNT(DISTINCT employee_id) AS cnt FROM accounts_payroll WHERE fiscal_year = $1`,
          [fiscalYear],
        )
        .catch(() => ({ rows: [{ cnt: 0 }] })),
  ...
  async createTransaction(payload, req) {
    ...
    if (!amount || Number(amount) <= 0) {
      throw Object.assign(new Error("Transaction amount must be positive"), {
        status: 400,
      });
    }
  ...
  async updateTransaction(id, payload, req) {
    ...
    if (posted.rows.length) {
      throw Object.assign(
        new Error(
          "Posted accounting transactions cannot be edited; create a reversal instead",
        ),
        { status: 409 },
      );
    }
  ...
  async deleteTransaction(id, req) {
    ...
    if (posted.rows.length) {
      throw Object.assign(
        new Error(
          "Posted accounting transactions cannot be deleted; create a reversal instead",
        ),
        { status: 409 },
      );
    }
    const result = await db.query(
      "DELETE FROM accounts_transactions WHERE id = $1 RETURNING id",
      [id],
    );
    if (!result.rows.length) throw new Error("Transaction not found");
  ...
  async getCollectionByClass(filters, req) {
    ...
    return result.rows.map((row) => ({
      ...
      pct: row.billed > 0 ? Math.round((row.collected / row.billed) * 100) : 0,
    }));
  }
